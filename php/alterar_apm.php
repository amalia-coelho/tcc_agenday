<?php
    ...
    try{
        ...
        if ($_FILES['alterarImagem']['name'] != null) {
            ...
            $arquivo = 'uploads/apm/' . $_FILES['alterarImagem']['name']; //Calcular destino imagem
        }else{
            $arquivo = $_POST['imagemAntiga'];
        }

        $stmt_update = $conn->prepare("UPDATE tb_apm SET nm_produto = :nome, ds_descricao = :descricao, ds_imagem = :ds_imagem,  nr_valor = :valor WHERE cd_apm = :codigo");
